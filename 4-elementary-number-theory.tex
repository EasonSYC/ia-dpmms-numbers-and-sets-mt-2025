...
\begin{corollary}[Solution to Congruence involving Unit is Unique]
    If \(\gcd\para{a, n} = 1\), then the congruence \(ax \equiv b \pmod{n}\) has a unique solution. In particular, if \(\gcd\para{a, n} = 1\), there exists a unique inverse of \(a\) modulo \(n\).
\end{corollary}
...
Therefore, if \(\gcd\para{a, n} = d > 1\), the congruence \(ax \equiv b \pmod{n}\) has \emph{no solution} unless \(d \divides b\), in which case the solution are exactly
\[
...
\begin{proof}
    ...
    \textbf{Existence.} Since \(\gcd\para{n, m} = 1\), by B\'{e}zout's Identity, there are \(s, t \in \ZZ\), such that \(sm + tn = 1\).
...
\begin{proof}
    ...
    Hence, the numbers \(a, 2a, 3a, \ldots, \para{p - 1}a\) are pairwise distinct (incongruent) under modulo \(p\), and are all non-zero modulo \(p\).
