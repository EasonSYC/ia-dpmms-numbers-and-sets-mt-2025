...
\begin{theorem}[Countability, Injection and Surjection]
    ...
    \begin{enumerate}
        \item \(X\) is countable;
        \item there is a injection \(X \to \NN\);
...
\begin{corollary}[Transcedental Numbers are Uncountable]
    There are uncountably many transcedental numbers.
\end{corollary}

\begin{proof}
    Let \(\mathcal{A}\) be the set of algebraic numbers, then \(\RR \setminus \mathcal{A}\) is the set of transcedental numbers.
