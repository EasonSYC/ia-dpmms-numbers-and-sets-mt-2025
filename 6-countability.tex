...
\begin{proof}
    ...
    \]
    where \(x = a_j^{\pqty{i}}\) for the least \(i\) such that \(x \in A_i\) -- this is guarenteed to exist by well ordering principle, i.e.,
    \[
...
\begin{proof}
    ...
    Simlilarly, \(g\) bijects from \(B_0\) to \(A_1\), observing that every \(a \in A_1\) has at least one ancestor, so is equal to \(g\pqty{b}\) for some \(b \in B_0\).
